...
pub use normalize::{
    candles_have_interior_gap, candles_have_interval_discontinuity, candles_have_missing_intervals,
...
pub(crate) enum CandleFetchPolicy {
    CacheFirst,
    NetworkOnly,
}

/// Parameters for a chart-history request.
///
...
pub(crate) struct ChartCandleFetchRequest {
    ...
    pub(crate) policy: CandleFetchPolicy,
}

impl CandleFetchPolicy {
...
pub async fn fetch_candles(
    ...
    fetch_hyperliquid_candles(
        ...
        start_time,
        end_time,
        CandleFetchPolicy::CacheFirst,
    )
    .await
}

pub(crate) async fn fetch_chart_backfill_candles(
...
fn candle_interval_ms(interval: &str) -> Option<u64> {
    Timeframe::from_api_str_opt(interval)
        .filter(|timeframe| timeframe.uses_candle_backfill() && *timeframe != Timeframe::Mo1)
