...
- `Top 8 by 24h Gain %` ranks visible and user-selectable perpetual markets by
  positive percentage change from `prevDayPx` to `markPx`. Markets without a
  positive finite gain, plus spot and outcome markets, are excluded.
...
top row and ranks five through eight the bottom row. Several perpetual dexes can
list the same underlying asset (HIP-3), so contracts for one asset are collapsed
to the single highest-24h-volume contract before ranking; each coin appears at
most once in the grid. Incomplete market-family responses and responses with
fewer than eight ranked markets leave the current workspace unchanged.
